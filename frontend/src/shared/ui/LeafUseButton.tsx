...
export interface LeafUseButtonProps {
  /** 지금 가진 연잎. 1 장 이상일 때만 이 버튼을 그린다 */
  count: number;
  /** 「연잎 한 장으로 」 뒤에 붙는 말. 예: `답변 받기` */
  action: string;
  disabled?: boolean;
  onClick: () => void;
 ...
 * 「연잎 한 장으로 ~하기」.
 *
 * **쓰고 나면 몇 장 남는지를 누르기 전에 적는다.** 마지막 한 장을 쓰는 것인지 모른 채
...
/**
 * 연잎이 있을 때 **아래로 내려가는** 광고 버튼.
 *
 * 순서를 뒤집지 않는다. 이미 값을 치러 둔 사람 앞에 광고를 먼저 세우면, 연잎을 모을
