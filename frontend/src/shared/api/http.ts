...
/**
 * 광고를 보기로 하고 실제로 본 사실. 다음 `/concern` 한 번만 들고 간다.
 *
 * ⚠ **「끝까지」는 아니다.** 이어가기는 광고를 틀고 5초가 지나면 답을 만들기 시작하고
 * 그때 이 표를 세운다. 광고가 한 장도 안 온 경우(`noFill`)에도 세운다. 우리 쪽 사정으로
 * 사람을 막지 않기로 했기 때문이다. 완주 판정(`rewarded_ad_complete`)은 이 값과 별개다.
 *
