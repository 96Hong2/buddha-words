 ...
 * 연잎을 세는 자리.
 *
 * 연잎 한 장이 광고 한 편이다. **미리 모아 두면 그때 광고를 안 봐도 된다.**
 * 이야기를 이어갈 때와 답을 간직할 때 한 장씩 쓴다.
 *
 * ── 왜 만들었나 ───────────────────────────────────────────────────────────
 *
 * 전에는 광고를 볼 자리가 곧 광고를 봐야 하는 자리였다. 답을 기다리는 사람 앞에
 * 30초가 서 있으면, 그 30초는 언제나 방해다. 연잎은 그 30초를 **사람이 한가할 때로**
 * 옮긴다. 낼 값이 줄지는 않지만 내는 시점을 고를 수 있다.
 *
 * 처음 한 장을 그냥 준다. 두 번째 이야기에서 광고 대신 연잎이 먼저 쓰이므로,
 * 이 앱에 연잎이라는 것이 있다는 사실을 설명이 아니라 경험으로 알게 된다.
 *
 * ── 저장 자리 ─────────────────────────────────────────────────────────────
 *
 * localStorage 다. 브릿지 저장소는 비동기라 화면이 한 번 그려진 뒤에 값이 오고,
 * 그러면 잔액이 0에서 3으로 튀거나 「연잎으로 받기」 버튼이 깜빡 떴다 사라진다.
...
function write(next: LeafState): void {
  ...
  try {
    ...
    // 저장이 막힌 기기에서는 이 세션 동안만 기억한다. 못 저장했다고 쓰던 연잎을 뺏지 않는다
  }
...
/** 지금 가진 연잎. 화면이 가장 자주 묻는 것이라 따로 둔다 */
