 ...
 *
 * ⚠ 한 번 쓴 첫 이야기는 **어느 쪽에서도 안 풀린다.** 서버 장부는 2026-09-23 에 저장
 * 자리로 내려가 배포를 넘게 됐지만(계획 X44), 그렇다고 이 단조성을 풀지 않는다. 저장
 * 자리가 잠깐 넘어지면 서버는 그동안 「안 썼다」로 답할 수 있고, 그 값을 그대로 덮으면
 * 기기 사본까지 풀려서 **그 창 안에 무료가 되살아난다.** 이 앱이 막으려는 바로 그 증상이다.
 *
