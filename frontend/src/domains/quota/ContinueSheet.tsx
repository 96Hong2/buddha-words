 ...
 * ── 이 자리 광고는 전면형이다 ─────────────────────────────────────
 *
 * 5초 뒤에 닫아도 답을 주므로 보상형을 쓰면 끝까지 안 본 사람에게 보상을 준 셈이 된다.
 * 앱인토스 규칙 위반이다. 보상형 30초가 실기기에서 너무 길다는 말도 있었다.
 * 전면형은 보상 없이 닫힘으로 끝나서 `watched` 가 오지 않는다. 5초 규칙만으로 가른다.
 */
        ...
        <div className="continue-sheet__actions">
          ...
          >
            ...
            {/*
              ...
              초는 **아직 적지 않는다.** 보상형일 때는 30초를 재서 적었는데, 전면형이 몇 초
              떠 있는지는 문서에 없다. 모르는 초를 적으면 화면이 지킬 수 없는 약속을 한다.
              `ad_close(placement=continue)` 로 실제 길이를 재고 나서 그 값을 적는다.
            */}
