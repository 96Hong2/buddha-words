 ...
 *
 * 여는 곳은 홈 연잎 칩 하나다. 이어가기·간직 시트에서 이리로 오는 길은 두지 않는다.
...
export function LeafSheet({ open, onClose }: LeafSheetProps) {
  ...
  /*
    ...
    다음에 열었을 때 아무것도 안 했는데 「연잎 한 장이 늘었어요」가 먼저 보인다.
  */
  useEffect(() => {
    setJustEarned(false);
    setFailed(null);
    ...
    <BottomSheet open={open} onClose={onClose} ariaLabel={TITLE} className="leaf-sheet">
      <div {...testId(TEST_IDS.leafSheet)}>
        <div className="leaf-sheet__balance">
          ...
          {/* 아이콘이 `aria-hidden` 이라, 무엇의 몇 장인지는 이 라벨이 말해야 한다 */}
          <p
            className="leaf-sheet__count"
            aria-label={`가진 연잎 ${count}장`}
        ...
        */}
        <p className="leaf-sheet__sub">
          {canCollect ? (
            ...
            <>
              가진 연잎으로 <b>광고 없이</b> 이야기를 이어가거나 말씀을 간직할 수 있어요.
            </>
          )}
        </p>
