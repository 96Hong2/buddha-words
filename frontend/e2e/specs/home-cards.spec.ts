...
/**
 * 지난번에 답을 받은 기기. 남는 것은 이 네 자리뿐이고 고민 원문도 답변 본문도 여기 없다.
 *
 * 한 번만 심는다. 열 때마다 다시 심으면 앱이 지운 것까지 되살아나, 다시 열어도 카드가
 * 안 뜨는지 확인할 수 없다.
 */
...
test('어제 답을 받았으면 회고 카드가 그날 적어 드린 행동을 되묻는다', async ({ page }) => {
  ...
  await shot(page, '07 홈 - 어제 이야기를 되짚는 회고 카드', { fullPage: true });
});
...
test('회고 카드는 한 번만 눌러도 답이 되고 다시 열어도 또 묻지 않는다', async ({ page }) => {
  await seedRecall(page);
  await page.goto('/');
  // 진입 카드를 치우면 입력칸으로 초점이 간다. 실제 사용자가 보는 순서 그대로다
  await page.getByTestId('entry-card-cta').click();

  const card = page.getByTestId('recall-card');
  await expect(card).toBeVisible();
  await card.getByRole('button', { name: '해봤어요' }).click();
  ...
  await expect(page.getByTestId('recall-card')).toHaveCount(0);
});
