...
export const EVENTS = {
  ...
  */
  ...
  /**
   * 광고가 뜬 채로 앱이 끝났다. **다음에 앱을 열 때 찍힌다.**
   *
   * ── 왜 다음 실행에서 찍나 ────────────────────────────────────────────
   *
   * 직접 눌러서 하는 광고(게임을 해 보는 형식)가 멈춰서 닫기 X 도 안 먹는 일이 있다
   * (2026-09-25 신고). 그 광고를 닫는 길은 우리에게 없다. 토스 앱이 띄운 화면이고
   * SDK 에 닫는 함수가 없다. 우리가 할 수 있는 것은 **얼마나 자주 나는지 세는 것**뿐이고,
   * 그 수가 콘솔에 신고할 근거이자 광고 자리를 줄일지 정하는 근거다.
   *
   * 그런데 그 판은 **살아 있는 동안 아무 로그도 못 남긴다.** 갇힌 사람은 앱을 끄고 나가고,
   * 우리 쪽 시간 제한(90초)이 `show_timeout` 을 찍기 전에 웹뷰가 죽는다. 그래서 광고가
   * 뜨는 순간 저장소에 표를 하나 적고, 정상적으로 끝나면 지운다. 다음에 앱을 열었을 때
   * 그 표가 남아 있으면 **지난번에 광고가 뜬 채로 끝났다**는 뜻이다.
   *
   * ⚠ 이 수에는 광고를 보다가 그냥 앱을 끈 사람도 섞인다. 고장만 세는 것이 아니다.
   * 자리별로 갈라 보고 **튀는 자리**를 찾는 데 쓴다.
   */
  ad_stuck_exit:       { params: ['placement'] as const },
  /*
