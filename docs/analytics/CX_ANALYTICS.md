...
| `ad_close` | 광고가 닫혔을 때. 뜬 순간부터 몇 초였나(`shown_bucket_ms`) | **버튼에 적을 초를 잰다.** 이어가기(전면형)는 이것이 유일한 노출 완료 신호다 |
...
| `continue` | 같은 날 두 번째 이야기 | **광고가 도는 동안 답을 만든다.** 끝나면 답이 와 있다. **전면형**이다(2026-09-20) |
| `save` | 간직하기를 누를 때 | 누른 뒤 곧바로 끝나 다음 화면이 없던 자리 |

버튼 라벨에는 **「광고」**가 들어가고, 보상형 두 자리는 몇 초짜리인지도 적는다(「30초」). 몇 초짜리인지
모르면 사람은 중간에 닫는다. 이어가기는 전면형이라 초를 적지 않는다. 길이가 문서에 없고,
광고를 닫아도 답이 나오니 참아야 할 시간을 약속할 이유도 없다.

**이어가기를 전면형으로 바꾸고 답과 떼었다(2026-09-20).** 보상형 30초가 실기기에서 너무
길었다. 광고를 봐야 답을 주는 구조는 보상형에만 허용되므로(앱인토스 「광고 소비를 보상과 직접
연결하는 구조 금지」), 전면형에서는 광고를 닫아도 답이 나온다. 버튼도 「광고 보고 답변 받기」가
아니라 「답변 받기(광고)」다. 전면형은 보상 이벤트가 없어서 `rewarded_ad_complete(placement=continue)`
는 0 이 정상이다. 광고 수와 이탈 지표의 분모에는 `ad_close(placement=continue)` 를 더한다.
이어가기의 `ad_close` 는 광고 길이가 아니라 사람이 닫은 시각이다. 버튼 문구의 초로 쓰지 않는다.
...
| `reason` | 뜻 | 할 일 |
| --- | --- | --- |
| `no_group` | **콘솔이 광고 그룹 id 를 아직 안 줬다.** 번들에 값이 없다 | 콘솔에서 그룹을 만들고 다시 빌드한다. 보상형 두 자리는 `VITE_AD_GROUP_DEFAULT`, 이어가기는 `VITE_AD_GROUP_CONTINUE`(전면형) |
...
⚠ `continue` 는 **가드레일을 같이 봐야 하는 자리다.** 광고가 뜬 순간 답을 만들기 시작하고,
광고를 곧바로 닫아도 답은 나온다. 광고와 답을 묶지 않는다(아래). 곧바로 닫는 사람
(`ad_close(placement=continue, shown_bucket_ms=<2s)`)이 많으면 광고가 거슬린다는 뜻이다.
...
| `VITE_AD_GROUP_DEFAULT` | 없음 | 콘솔이 발급한 보상형 광고 그룹 id. **없으면 보상형 두 자리가 광고 없이 지나간다**(`ad_skipped(reason=no_group)`) |
| `VITE_AD_GROUP_CONTINUE` | 없음 | 이어가기 전면형 광고 그룹 id. 공용으로 떨어지지 않는다. 광고 그룹을 주면서 이것만 빠뜨리면 번들 검사가 빌드를 막는다 |
