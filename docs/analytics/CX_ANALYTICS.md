...
버튼 라벨에는 **「광고」**가 들어가고, 잰 초가 있으면 함께 적는다(보상형 「30초」). 몇 초짜리인지
모르면 사람은 중간에 닫는다. 이어가기는 전면형 길이를 재기 전이라 초를 비워 두었다.
KPI `continue_ad_length` 에서 가장 많은 구간을 보고 그 값을 적는다.

**이어가기를 전면형으로 바꿨다(2026-09-20).** 보상형 30초가 실기기에서 너무 길었고, 5초 뒤
닫아도 답을 주는 구조가 보상형 규칙(`userEarnedReward` 때만 보상)과 맞지 않았다. 전면형은
보상 이벤트가 없어서 `rewarded_ad_complete(placement=continue)` 는 0 이 정상이다. 광고 수와
이탈 지표의 분모에는 `ad_close(placement=continue)` 를 더한다.
