...
class DayUsage:
    ...
    light_used: int = 0
    # 멱등키 → 지난번 결과. 같은 요청이 두 번 와도 두 번 세지 않는다
    seen: dict[str, Gate] = field(default_factory=dict)
...
def _day(anon_key: str, zone: ZoneInfo, now: datetime | None = None) -> DayUsage:
    key = (anon_key, today_in(zone, now).isoformat())
...
def reserve(
    ...
    같은 멱등키로 다시 오면 지난번 판정을 그대로 돌려주고 다시 세지 않는다.
    """
    day = _day(anon_key, zone, now)
    ...
    if idempotency_key and idempotency_key in day.seen:
        gate = day.seen[idempotency_key]
        return Outcome(gate != "exhausted", gate, snapshot(anon_key, zone, now))
