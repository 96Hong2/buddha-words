...
# 앱당 분당 15,000회 한도(공식 문서). 여유를 두고 기본값을 낮게 잡는다.
# 사용자 한 명 기준으로는 분당 10회라, 같은 사람에게 몰아 보내지 않는 것은 부르는 쪽이 지킨다.
...
class MisconfiguredTossApiError(RuntimeError):
...
class TossApiSettings:
    ...
    def cert_tuple(self) -> tuple[str, str]:
        if not self.has_client_certificate:
            raise MisconfiguredTossApiError(
                "토스 서버 API 는 mTLS 클라이언트 인증서가 있어야 호출할 수 있다. "
                "docs/DEPLOY.md 의 mTLS 절을 본다."
            )
