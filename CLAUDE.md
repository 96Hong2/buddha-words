...
4. 광고는 `AdPlacement` 세 자리(`extension` · `continue` · `save`)뿐. **셋 다 사람이 버튼을 눌러야 뜬다. 저절로 뜨는 광고를 만들지 않는다.** 버튼 라벨에는 「광고」가 들어가고, 잰 초가 있으면 초도 함께 적는다(보상형 「30초」). 완주 판정(`rewarded_ad_complete`)은 `userEarnedReward` 에서만. **이어가기는 전면형이다**(`AD_KIND`, 그룹은 `VITE_AD_GROUP_CONTINUE` 만, 공용 보상형 그룹으로 떨어뜨리지 않는다). 광고를 틀고 5초가 지나면 답 생성을 먼저 시작하고 그 뒤에 닫아도 답을 주므로, 보상형으로 두면 규칙 위반이다. 광고가 안 오면(`noFill`) 막지 말고 그냥 보낸다. 입력·위기 화면에 광고 없음. 첫 답에는 간직 광고를 면제한다(`answersTotal <= 1`).
