...
4. 광고는 `AdPlacement` 세 자리(`extension` · `continue` · `save`)뿐. **셋 다 사람이 버튼을 눌러야 뜬다. 저절로 뜨는 광고를 만들지 않는다.** 버튼 라벨에는 「30초」와 「광고」가 함께 들어간다. 보상은 `userEarnedReward` 에서만. 입력·위기 화면에 광고 없음. **첫 답까지는 어느 자리에서도 안 띄운다**(`isFirstStory`).
