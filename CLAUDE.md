...
4. 광고는 `AdPlacement` 세 자리(`extension` · `continue` · `save`)뿐. **셋 다 사람이 버튼을 눌러야 뜬다. 저절로 뜨는 광고를 만들지 않는다.** 버튼 라벨에는 「광고」가 들어가고, 잰 초가 있으면 초도 함께 적는다(보상형 「30초」). 완주 판정(`rewarded_ad_complete`)은 `userEarnedReward` 에서만. **이어가기는 전면형이고 답과 떼어 놓는다**(`AD_KIND`, 그룹은 `VITE_AD_GROUP_CONTINUE` 만, 공용 보상형 그룹으로 떨어뜨리지 않는다). 광고가 뜬 순간 답 생성을 시작하고, 광고를 곧바로 닫아도 답은 나온다. **광고를 봐야 무언가를 주는 구조는 보상형에만 허용된다**(앱인토스 「광고 소비를 보상과 직접 연결하는 구조 금지」). 전면형에 시청 조건을 걸지 않고, 버튼에 「광고 보고 ~받기」를 쓰지 않는다. 광고를 불러오는 동안은 화면을 넘기지 않는다(뒤늦은 광고가 답을 덮지 않게). 광고가 안 오면(`noFill`) 막지 말고 그냥 보낸다. 입력·위기 화면에 광고 없음. 첫 답에는 간직 광고를 면제한다(`answersTotal <= 1`).
