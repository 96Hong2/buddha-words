...
4. 광고는 `AdPlacement` 세 자리(`extension` · `continue` · `save`)뿐. **셋 다 사람이 버튼을 눌러야 뜬다. 저절로 뜨는 광고를 만들지 않는다.** 버튼 라벨에는 「30초」와 「광고」가 함께 들어간다. 완주 판정(`rewarded_ad_complete`)은 `userEarnedReward` 에서만. 단 **이어가기는 광고를 틀고 5초가 지나면 답 생성을 먼저 시작**한다(광고가 끝나자마자 답을 보여주려고). 광고가 안 오면(`noFill`) 막지 말고 그냥 보낸다. 입력·위기 화면에 광고 없음. 첫 답에는 간직 광고를 면제한다(`answersTotal <= 1`).
